...
export default function App() {
 ...
 const constructDegreeData = (courses) => {
   ...
   courses.forEach((c) => {
     // Extract year from term, e.g. "FA23" => "FA", 2023
     const semMatch = c.term.match(/([A-Z]+)(\d{2})/);
     if (!semMatch) return;
    
     const semester = semMatch[1];
     const year = `20${semMatch[2]}`; // "FA23" => "2023"
   ...
   // Ensure the semester array exists
   if (!data["2025"]["Spring"]) data["2025"]["Spring"] = [];


   // DUMMY PLANNED COURSE OBJECT
 ...
 const constructUserReq = (degreeData) => {
   const userReq = {};
    Object.entries(sdsuReq).forEach(([reqKey, req]) => {
     userReq[reqKey] = [];
      // Flatten all classes in this requirement (class_codes + subareas)
     const allClasses = [];
     ...
     if (req.subarea) {
      ...
      // Go through all courses in degreeData
 ...
 // Function to build requirement progress summary
 ...
 // Function to build "to-do" requirement object
 const constructTodoReq = (priorReq) => {
   const todoReq = {};


   Object.entries(priorReq).forEach(([reqKey, req]) => {
     const remaining = Math.max(req.target - req.completed, 0); // don't allow negative
     todoReq[reqKey] = {
       name: req.name,
